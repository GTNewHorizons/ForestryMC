...
public interface IMultiblockComponent {
    ...
    /**
     * Called when a machine is fully assembled from the disassembled state, meaning it was constructed by a
     * player/entity action, not by chunks loading. Note that, for non-square machines, the min/max coordinates may not
     * actually be part of the machine! They form an outer bounding box for the whole machine itself.
     * 
     * @param multiblockController The controller to which this part is being assembled.
     */
    void onMachineAssembled(IMultiblockController multiblockController, ChunkCoordinates minCoord,
