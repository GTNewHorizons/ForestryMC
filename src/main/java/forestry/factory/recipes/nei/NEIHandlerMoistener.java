...
public class NEIHandlerMoistener extends RecipeHandlerBase {
    ...
    public class CachedMoistenerRecipe extends CachedBaseRecipe {
        ...
        public CachedMoistenerRecipe(IMoistenerRecipe recipe, MoistenerFuel fuel) {
            this.tank = new PositionedFluidTank(
                    FluidRegistry.getFluidStack("water", 10000),
                    10000,
                    new Rectangle(11, 5, 16, 58),
                    NEIHandlerMoistener.this.getGuiTexture(),
                    new Point(176, 0));
    ...
    public String getRecipeName() {
        return StringUtil.localizeGuiTitle("tile.for.factory.4.name");
    }
    ...
    public void loadCraftingRecipes(ItemStack result) {
        for (IMoistenerRecipe recipe : RecipeManagers.moistenerManager.recipes()) {
            for (MoistenerFuel fuel : FuelManager.moistenerResource.values()) {
                if (NEIServerUtils.areStacksSameTypeCrafting(recipe.getProduct(), result)
                        || NEIServerUtils.areStacksSameTypeCrafting(fuel.product, result)) {
                    this.arecipes.add(new CachedMoistenerRecipe(recipe, fuel));
                }
            }
        }
    ...
    public void loadUsageRecipes(ItemStack ingred) {
        ...
        for (IMoistenerRecipe recipe : RecipeManagers.moistenerManager.recipes()) {
            for (MoistenerFuel fuel : FuelManager.moistenerResource.values()) {
                if (NEIServerUtils.areStacksSameTypeCrafting(recipe.getResource(), ingred)
                        || NEIServerUtils.areStacksSameTypeCrafting(fuel.item, ingred)) {
                    this.arecipes.add(new CachedMoistenerRecipe(recipe, fuel));
                }
            }
        }
