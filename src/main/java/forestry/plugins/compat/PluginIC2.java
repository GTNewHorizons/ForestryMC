...
@Plugin(
        pluginID = "IC2",
        name = "IndustrialCraft2",
        author = "SirSengir",
        url = Constants.URL,
        unlocalizedDescription = "for.plugin.ic2.description")
public class PluginIC2 extends ForestryPlugin {
    ...
    protected void registerRecipes() {
        ...
        if (energyBlocks != null) {
            RecipeUtil.addRecipe(
                    energyBlocks.engine.get(BlockEngineType.GENERATOR),
                    "X#X",
                    "XYX",
                    "X#X",
                    '#',
            ...
            RecipeUtil.addRecipe(
                    energyBlocks.engine.get(BlockEngineType.ELECTRIC),
                    "###",
                    " X ",
                    "YVY",
                    '#',
    ...
    public ArrayList<ItemStack> getCropDrops(TileEntity tileEntity) {
        if (isIC2Crop(tileEntity)) {
            ICropTile crop = (ICropTile) tileEntity;
            if (crop == null || crop.getCrop() == null) return null;
            ItemStack[] cropDrops;
            if ((crop.getCrop().canGrow(crop) && crop.getSize() < crop.getCrop().getOptimalHavestSize(crop))
                    || crop.getSize() == crop.getCrop().getOptimalHavestSize(crop))
                cropDrops = crop.harvest_automated(true);
            else cropDrops = crop.harvest_automated(false);
            if (cropDrops != null) {
